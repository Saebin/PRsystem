				...
				<div class="row">
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">나의 과제현황</div>
							<!-- /.panel-heading -->
								...
								<div class="dataTable_wrapper">
									<table class="table table-striped table-bordered table-hover"
										...
										<tbody>
											...
											<tr>
												<td></td>
												<td><form action="taskdetail" method="POST">
												<input type="hidden" name="num01" value="<%=taskInfoList.get(i).getTask_number() %>">													
												<input type="submit" class="similarity" value="<%=taskInfoList.get(i).getTask_name()%>">												
												</form></td>
												<td>
												...
												<input type="hidden" name="num01" value="<%=taskInfoList.get(i).getTask_number() %>">													
												<input type="submit" class="similarity" value="제출하기">												
												</form></td>
												<td>
												<%for(int j=0; j<taskEvaluateManagementInfo.size(); j++){ 
													if(taskEvaluateManagementInfo.get(j).getStd_id() == stdInfo.getStdId() && taskEvaluateManagementInfo.get(j).getTask_number() == taskInfoList.get(i).getTask_number()){
														count++;								
												}}%>
												<form action="taskevaluation" method="POST">
												<input type="hidden" name="num" value="<%=taskInfoList.get(i).getTask_number() %>">	
												<input type="hidden" name="code" value="<%=taskInfoList.get(i).getSubj_code_div() %>">
												<input type="hidden" name="std_id" value="<%=stdInfo.getStdId()%>">													
												<input type="submit" class="similarity" value="<%=count %>/4">												
												</form></td>
											</tr>
											<!-- Table Row -->
											<%} %>
